...
void solve(std::ifstream &inputFile, std::ostream *output)
{
    ...
    std::string pref_mul = "mul(", pref_do = "do()", pref_dont = "don't()";
    const int MUL_SIZE = pref_mul.size(),
              DO_SIZE = pref_do.size(),
              DONT_SIZE = pref_dont.size();
    long long total = 0;
    bool accept = true;
    while (std::getline(inputFile, line))
    {
        std::vector<int> p = kmp(pref_mul + line),
                         p_do = kmp(pref_do + line),
                         p_dont = kmp(pref_dont + line);
        for (int i = 0; i < line.length(); i++)
        {
            if (p_do[i + DO_SIZE] == DO_SIZE)
                accept = true;
            if (p_dont[i + DONT_SIZE] == DONT_SIZE)
                accept = false;
